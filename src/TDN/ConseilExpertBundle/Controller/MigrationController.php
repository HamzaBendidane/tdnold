...
die;
	...
	public function migrationV1IDAction($fichier) {
		...
		set_time_limit(0);
       	$em = $this->get('doctrine.orm.entity_manager');
        // Recherche de rôle
        $rep_articles = $em->getRepository('TDN\ConseilExpertBundle\Entity\ConseilExpert');
        ...
        try {
			...
			if ($handle !== FALSE) {
				...
				$champs = array_flip($keys);
		    	while (($row = fgetcsv($handle, NULL, ",")) !== FALSE) {
		    		$done = $rep_articles->findOneByV2ID($row[$champs['id_conseil']]);
		    		if ($done instanceof ConseilExpert) {
		    			$done->setV1ID('c.'.$row[$champs['id_v1']]);
		    			$em->flush();
		    			print_r(" + ".$row[$champs['id_conseil']].$done->getTitre().'<br>');
		    		} else {
		    			print_r(" - ".$row[$champs['id_conseil']]." introuvable</br>");
		    		}
		    	}
	        }
        } catch (\Exception $e) {
