...
class ArticleRepository extends DocumentRepository
{
	...
	public function findRedactionRecent($limite, $panel = NULL)
	{
        ...
        $rsm->addEntityResult('TDN\DocumentBundle\Entity\Document', 'a');
        ...
        // $rsm->addJoinedEntityResult('TDN\DocumentRubiqueBundle\Entity\DocumentRubrique' , 'r', 'n', 'roles');
...
print_r($sql);
      ...
      public function findBonsPlans($limite,$debut) {
		...
		$query = $qb->select('u')
					->where($qexpr->andX(
						$qexpr->like('u.statut', $qb->expr()->literal('ARTICLE_PUBLIE')),
						$qexpr->like('u.titre', ':seed')))
					->setParameter('seed', 'bon plan%')
					->orderBy('u.idDocument', 'DESC')
