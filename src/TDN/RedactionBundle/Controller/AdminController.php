...
class AdminController extends DocumentController {
	...
	public function articleAddAction () {
		...
		$this->initFormulaireThematiques($_TDNDocument);
		$variables['formThematiques'] = $this->twigVars['formThematiques'];
		...
		if ($request->getMethod() === 'POST') {
			...
			if ($form->isValid()) {
				...
				// $sliderManager = $this->get('tdn.slider_manager');
				if (!($miseEnAvant == 1)) {
				...
				$this->formulaireThematiques->bindRequest($request);
				$_TDNDocument->assigneRubriques($this->themes);
