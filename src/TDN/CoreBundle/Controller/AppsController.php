...
class AppsController extends Controller {
    ...
    private function _makeJSON ($resultats) {
...
*/        $patterns[0] = '/\"/';
    ...
    private function getVideos ($limite, $panel = NULL,$debut)
    {
        ...
        foreach ($_articles as $a) {
            ...
            if ($a->getIdHebergeur() != "youtube" ) continue;
	...
	private function getAppArticles ($limite, $panel = NULL) 
    {
        $em = $this->get('doctrine.orm.entity_manager');
	...
	private function getAppConseils ($limite, $panel = NULL) 
    {
		$em = $this->get('doctrine.orm.entity_manager');
	...
	private function getAppBreves ($limite, $panel = NULL) 
    {
        $em = $this->get('doctrine.orm.entity_manager');
	...
	private function getAppPosts ($limite, $panel = NULL) 
    {
        $em = $this->get('doctrine.orm.entity_manager');
    ...
    public function iPhoneFluxAction () 
    {
        ...
        $res = array();
        if ($type == 0) {
            $res['flux'] =  $this->getAppPosts($_limite, $panel);
        } else {
            if ($type >= 4) {
                $type -= 4;
                $res['flux'] = $this->getAppArticles($_limite, $panel);
            }
            if ($type >= 2) {
                $type -= 2;
                $res['flux'] = $this->getAppConseils($_limite, $panel);
            }
            if ($type >= 1) {
                $res['flux'] =  $this->getAppBreves($_limite, $categorie);
            }            
        }
    ...
    // Derniers conseils d'experts
    public function iOSDerniersConseilsAction () {

        $conseil['entite'] = 'TDN\ConseilExpertBundle\Entity\ConseilExpert';
        $conseil['route'] = 'ConseilExpert_conseil';

        return $this->iOSDerniersContenusAction($conseil);
    }

    // Derniers conseils d'experts
    public function iOSDernieresQuestionsAction () {

        $question['entite'] = 'TDN\CauseuseBundle\Entity\Question';
        $question['route'] = 'CauseuseBundle_conversation';

        return $this->iOSDerniersContenusAction($question);
    }

    public function iOSDerniersContenusAction (array $type) {
        $request = $this->get('request');
        $limite = $request->query->get('limite');
        $panel = NULL;
        $em = $this->get('doctrine.orm.entity_manager');
        ...
        $_articles = $rep->findMostRecent($limite, $panel);
        foreach ($_articles as $a) {
            $resultats[] = $this->extractHeader($a, $type['route']);
         }

        $response = new Response($this->_makeJSON($resultats));
        $response->headers->set('Content-Type', 'application/json');
        $response->headers->set('Accept-Charset', 'utf-8');
        return $response;
    }
    ...
    public function iOSVideosAction () 
    {
        ...
        $res = array();
        $res['flux'] =  $this->getVideos($_limite, $panel ,$_debut);
