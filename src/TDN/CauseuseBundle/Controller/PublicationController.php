...
class PublicationController extends DocumentController {
	
	public function questionAction ($rubrique, $slug, $id) {
		...
		return $this->render('CauseuseBundle:Page:questionNanas.html.twig', $variables);
	}
	...
	public function reponsePosterAction () {
		...
		if ($request->getMethod() == "POST") {
			...
			// Notification
			$adresse_valide = filter_var($_TDNDocument->getLnAuteur()->getEmail(), FILTER_VALIDATE_EMAIL);
			if ($adresse_valide) {
	...
	public function reponseVoterAction ($id) {
        ...
        if ($isBot === false) {
        	...
        	$reponse->getLnAuteur()->updatePopularite($points['reponse_vote']);
	...
	public function reponseAccepterAction ($idQuestion, $idReponse) {
		...
		$question->setReponseAcceptee($reponse->getIdDocument());
